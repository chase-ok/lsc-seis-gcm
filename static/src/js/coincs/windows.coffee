...
(utils, plots, d3, $, _) ->
    ...
    $ ->
        ...
        utils.loadUnwrappedJSON "/data/coinc/windows-#{group.id}.json", (data) ->
            utils.loadUnwrappedJSON "/data/coinc/windows-#{group.id}-without-exact.json", (dataWithout) ->
                # silly me in including a window size of 1000...
                data = (datum for datum in data when datum.window <= 100)

                plotField = (name, getValue, withoutExact) ->
                    scatter.axisLabels
                        y: name

                    actual = []
                    timeOffset = []
                    for datum in (if withoutExact then dataWithout else data)
                ...
                baseFields = [["Number of Coincidences", (d) -> d.num.overall_coincs],
